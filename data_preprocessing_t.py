...
# desc = desc['features']
#lap_pos = read_pickle("/tmp/PEB-DDI-main/PEB-DDI/dataset/trans/drugbank/drug_pos_data.pkl")
drug_dgl = read_pickle("/tmp/PEB-DDI-main/PEB-DDI/dataset/trans/drugbank/drug_dgl_data_zhang.pkl")
drugbank_similarity = read_pickle("/tmp/PEB-DDI-main/PEB-DDI/dataset/trans/drugbank/drug_similarity_zhang.pkl")
#drugbank_association = read_pickle('/tmp/PEB-DDI-main/PEB-DDI/dataset/trans/drugbank/drugbank_association_matrix.pkl')
ID_index = read_pickle('/tmp/PEB-DDI-main/PEB-DDI/dataset/trans/drugbank/ID_index.pkl')
drug_ID = ID_index
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if 'drugbank' not in dp_dataset_name:
            for h, t, r in batch:
                if r == 1:
                    # pos_h
                    pos_rels.append(r)
                    pos_h_samples.append(self.__create_graph_data(h,8))
                    pos_h_dgl.append(drug_dgl[h])
                    # pos_t
                    pos_t_samples.append(self.__create_graph_data(t,8))
                    pos_t_dgl.append(drug_dgl[t])
                    ...
                    pos_b_graph = self._create_b_graph(get_bipartite_graph(h_graph, t_graph),
                                                       self.__create_graph_data(h, 8).x,
                                                       self.__create_graph_data(t, 8).x)
                    pos_b.append(pos_b_graph)

                if r == 0:
                    # neg_h
                    neg_rels.append(r)
                    neg_h_samples.append(self.__create_graph_data(h,8))
                    neg_h_dgl.append(drug_dgl[h])
                    # neg_t
                    neg_t_samples.append(self.__create_graph_data(t,8))
                    neg_t_dgl.append(drug_dgl[t])
                    # neg_edge
                    neg_h_edge_samples.append(self.__create_graph_data_edge(h))
                    neg_t_edge_samples.append(self.__create_graph_data_edge(t))
                    h_graph = drug_to_mol_graph[h]
                    t_graph = drug_to_mol_graph[t]
                    neg_b_graph = self._create_b_graph(get_bipartite_graph(h_graph, t_graph),
                                                       self.__create_graph_data(h, 8).x,
                                                       self.__create_graph_data(t, 8).x)
                    neg_b.append(neg_b_graph)
        else:
            for h, t, r in batch:
                pos_rels.append(r)
                # pos_h
                ...
                h_graph = drug_to_mol_graph[h]
                t_graph = drug_to_mol_graph[t]
                pos_b_graph = self._create_b_graph(get_bipartite_graph(h_graph,t_graph),self.__create_graph_data(h,8).x,self.__create_graph_data(t,8).x)
                pos_b.append(pos_b_graph)
                neg_heads, neg_tails = self.__normal_batch(h, t, r, self.neg_ent)

                for neg_h in neg_heads:
                    neg_rels.append(r)
                    neg_h_samples.append(self.__create_graph_data(neg_h,8))
                    neg_h_dgl.append(drug_dgl[neg_h])
                ...
                for neg_t in neg_tails:
                    ...
                    neg_t_samples.append(self.__create_graph_data(neg_t,8))
                    neg_t_dgl.append(drug_dgl[neg_t])
        ...
        pos_h_dgl = dgl.batch(pos_h_dgl)

        pos_t_samples = Batch.from_data_list(pos_t_samples)
        pos_t_dgl = dgl.batch(pos_t_dgl)
        ...
        pos_tri = (pos_h_samples,pos_h_dgl, pos_t_samples,pos_t_dgl, pos_rels, pos_b, pos_h_edge_samples, pos_t_edge_samples)
        ...
        neg_h_dgl = dgl.batch(neg_h_dgl)

        neg_t_samples = Batch.from_data_list(neg_t_samples)
        neg_t_dgl = dgl.batch(neg_t_dgl)
        ...
        neg_tri = (neg_h_samples,neg_h_dgl, neg_t_samples,neg_t_dgl, neg_rels,neg_b, neg_h_edge_samples, neg_t_edge_samples)
    ...
    def __create_graph_data(self, id, pos_enc_dim):
        src_nodes1, dst_nodes1 = drug_dgl[id].edges()
        edge_index = torch.stack((src_nodes1, dst_nodes1), dim=0)
        n_features = drug_dgl[id].ndata['feat']
        edge_attr = drug_dgl[id].edata['feat']
        edge_list, edge_feats = (edge_attr[:, :2], edge_attr[:, 2:].float()) if len(edge_attr) else (
        ...
        # lap_pos_em = lap_pos[id]
        drug_index = drug_ID[id]
        return Data(x=n_features, edge_index=edge_index, edge_attr=edge_attr, id=drug_index, db=id,
